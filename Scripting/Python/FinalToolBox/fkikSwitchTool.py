...
class FKIKUI():
    ...
    def create(self):
        ...
        cmds.text('FKIK Switch: Select (ctrl + jnt + jnt + jnt, etc) and press EXECUTE')
    ...
    def FKIKSwitch(self, ctrlName='jointChain'):
        ...
        for x in range(0, jointCount):
            ...
            cmds.setDrivenKeyframe(sels[count] + '_parentConstraint2.' + fkChain[count - 1] + 'W0',
                                   cd=(sels[0] + '.' + ctrlName + 'FKIK'), dv=0, v=1)
            cmds.setDrivenKeyframe(sels[count] + '_parentConstraint2.' + fkChain[count - 1] + 'W0',
                                   cd=(sels[0] + '.' + ctrlName + 'FKIK'), dv=1, v=0)
            cmds.setDrivenKeyframe(sels[count] + '_parentConstraint2.' + ikChain[count - 1] + 'W1',
